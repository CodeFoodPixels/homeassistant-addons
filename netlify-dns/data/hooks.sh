...
function updateNetlify() {
  local TXT_VALUE="${1}"
  local DNS_ZONES_RESPONSE=$(curl -s -w "%{http_code}" "$NETLIFY_API/dns_zones?access_token=$SYS_TOKEN" --header "Content-Type:application/json")
  ...
  local DNS_RECORDS_RESPONSE=$(curl -s -w "%{http_code}" "$NETLIFY_API/dns_zones/$ZONE_ID/dns_records?access_token=$SYS_TOKEN" --header "Content-Type:application/json")
  ...
  if [[ "$RECORD_VALUE" != "$TXT_VALUE" ]]; then
    if [[ ! -z "$RECORD_VALUE" ]]; then
      ...
      local DELETE_RESPONSE_CODE=$(curl -X DELETE -s -w "%{http_code}" "$NETLIFY_API/dns_zones/$ZONE_ID/dns_records/$RECORD_ID?access_token=$SYS_TOKEN" --header "Content-Type:application/json")
    ...
    if [[ ! -z "$TXT_VALUE" ]]; then
      ...
      local CREATE_RESPONSE=$(curl -s -w "%{http_code}" --data "$CREATE_BODY" "$NETLIFY_API/dns_zones/$ZONE_ID/dns_records?access_token=$SYS_TOKEN" --header "Content-Type:application/json")
